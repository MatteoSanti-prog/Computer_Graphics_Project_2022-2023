...
void GameLogic(float deltaT, glm::vec3 m, glm::vec3 r, glm::mat4& ViewMatrix, glm::mat4& WorldMatrix, glm::vec3& CarPos, float& CarYaw, glm::vec3& CamPos) {
	...
	const float AccFactorForward = 1.0f;
	const float AccFactorBackward = -0.2f;
	const float DecFactorForward = 0.15f;
	const float DecFactorBackward = -0.1f;
	
	static float RotSpeed = glm::radians(90.0f);
	...
	if (MovSpeed > 0.0f) {
		LocalCarYaw += -RotSpeed * r.y * deltaT * (MovSpeed / MaxSpeedForward);
	}
	else if (MovSpeed < 0.0f) {
		LocalCarYaw += -RotSpeed * r.y * deltaT * (MovSpeed / MinSpeedBackward);
	}
